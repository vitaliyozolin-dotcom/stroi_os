...
export const loginPage = ({ username = '', error = '', blocked = false } = {}) => `<!doctype html>
<html lang="ru">
<head>
  <meta charset="utf-8">
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;min-height:100vh;display:grid;place-items:center;padding:24px;background:radial-gradient(circle at 15% 10%,#fff 0,transparent 34%),linear-gradient(145deg,#eef1eb,#f7f7f4)}
    ...
    button{width:100%;height:52px;border:0;border-radius:13px;background:#1f6b45;color:#fff;font:700 15px inherit;cursor:pointer;margin-top:4px}button:hover{background:#18583a}.error{padding:12px 14px;border-radius:12px;background:#fff0ed;color:#9d382a;font-size:13px;margin:-8px 0 18px}.foot{text-align:center;color:#879189;font-size:12px;margin-top:18px}
...
<body>
  <main class="shell">
    ...
    <section class="card">
      ...
      <p>Войдите в систему управления строительством.</p>
      ${error ? `<div class="error">${blocked ? 'Слишком много попыток. Повторите вход через 15 минут.' : 'Неверный логин или пароль.'}</div>` : ''}
      <form method="post" action="/api/auth/login">
        <div class="field"><label for="username">Логин</label><input id="username" name="username" value="${escapeHtml(username)}" autocomplete="username" required autofocus></div>
