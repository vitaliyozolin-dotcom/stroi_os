      ...
      bind: (...values: unknown[]) => ({
        run: async () => {
          ...
          if (normalized.startsWith('UPDATE telegram_updates')) {
            const [receivedAt, updateId, expectedStatus, expectedReceivedAt] = values.map(String);
...
test('first Telegram delivery atomically claims its update id', async () => {
  ...
  assert.deepEqual(result, { claimed: true, duplicate: false });
...
test('active and completed Telegram deliveries are treated as duplicates', async () => {
  ...
  assert.deepEqual(await claimTelegramUpdate(db, '102', { now: new Date(now.getTime() + 30_000) }), { claimed: false, duplicate: true });
  const row = db.rows.get('102');
  assert.ok(row);
  row.status = 'done';
  assert.deepEqual(await claimTelegramUpdate(db, '102', { now: new Date(now.getTime() + 60_000) }), { claimed: false, duplicate: true });
});
...
test('stale processing delivery can be reclaimed but only once', async () => {
  ...
  assert.deepEqual(await claimTelegramUpdate(db, '104', { now, processingTtlMs: 120_000 }), { claimed: true, duplicate: false });
  assert.deepEqual(await claimTelegramUpdate(db, '104', { now, processingTtlMs: 120_000 }), { claimed: false, duplicate: true });
});
