...
test('both runtime artifacts include extracted project, access, automation, integration and API boundaries', () => {
  const prepare = source('scripts/prepare-sites-build.mjs');
  const dockerfile = source('Dockerfile');
  for (const modulePath of ['access/session', 'access/users', 'automations/battle', 'feedback/routes', 'integrations/camera', 'integrations/notifications', 'integrations/routes', 'integrations/telegram-access', 'leads/routes', 'projects/routes', 'projects/write', 'routes/api']) {
