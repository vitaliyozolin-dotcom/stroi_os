...
import { BookOpen, ChevronLeft, ChevronRight, CircleDollarSign, FolderPlus, HelpCircle, ListTodo, UsersRound, X } from 'lucide-react';
...
export function HelpCenter({ onNavigate, onOpenProjects }: { onNavigate: (page: PageId) => void; onOpenProjects: () => void }) {
  ...
  const next = () => {
    if (!tour) return;
    if (stepIndex >= tour.steps.length - 1) { setTour(null); return; }
