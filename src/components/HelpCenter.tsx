...
type FeedbackItem = { id: string; createdAt: string; page: string; category: string; title: string; details: string; status: string };
...
export function HelpCenter({ projectId, currentPage, onNavigate, onOpenProjects, onCloseProjects }: { projectId: string; currentPage: PageId; onNavigate: (page: PageId) => void; onOpenProjects: () => void; onCloseProjects: () => void }) {
  ...
  return <>
    ...
    {open && <Modal title="Обучение СтройОС" subtitle={`${completed.length} из ${topics.length} разделов пройдено. Можно закрыть обучение — на его месте появятся «Правки».`} onClose={() => setOpen(false)}><div className="help-topics">{topics.map((topic) => { const Icon = topic.icon; const done = completed.includes(topic.id); return <button type="button" key={topic.id} onClick={() => activate(topic)}><span>{done ? <Check size={21} /> : <Icon size={21} />}</span><div><strong>{topic.title}</strong><p>{topic.text}</p></div><ChevronRight size={18} /></button>; })}<article><span><BookOpen size={21} /></span><div><strong>Главное правило СтройОС</strong><p>Каждая сумма и дата должны иметь основание: этап, заказ, приёмку и документ.</p></div></article><button type="button" className="help-dismiss-row" onClick={dismissTraining}><span><X size={20} /></span><div><strong>Закрыть обучение</strong><p>Вместо него появится раздел «Правки разработчику».</p></div><ChevronRight size={18} /></button></div></Modal>}
    {feedbackOpen && <Modal title="Правки разработчику" subtitle="Опишите проблему или улучшение. Текущий раздел и проект приложатся автоматически." onClose={() => setFeedbackOpen(false)}><form className="developer-feedback" onSubmit={submitFeedback}><div className="form-grid form-grid--2"><Field label="Тип"><select value={feedback.category} onChange={(event) => setFeedback((value) => ({ ...value, category: event.target.value }))}><option>Ошибка</option><option>Улучшение</option><option>Вопрос</option></select></Field><Field label="Раздел"><input value={currentPage} readOnly /></Field></div><Field label="Коротко"><input value={feedback.title} maxLength={160} required placeholder="Что нужно исправить" onChange={(event) => setFeedback((value) => ({ ...value, title: event.target.value }))} /></Field><Field label="Подробности"><textarea value={feedback.details} maxLength={3000} required rows={5} placeholder="Что произошло, что ожидали и как воспроизвести" onChange={(event) => setFeedback((value) => ({ ...value, details: event.target.value }))} /></Field><button type="submit" className="button button--primary"><Send size={16} /> Отправить разработчику</button>{feedbackStatus && <p className="developer-feedback__status">{feedbackStatus}</p>}</form>{feedbackItems.length > 0 && <section className="developer-feedback__history"><strong>Последние правки</strong>{feedbackItems.slice(0, 5).map((item) => <article key={item.id}><span>{item.category} · {item.status === 'new' ? 'новая' : item.status}</span><strong>{item.title}</strong><p>{item.details}</p></article>)}</section>}</Modal>}
