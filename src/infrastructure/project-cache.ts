...
import type { CacheWriteStatus, CachedProject, ProjectCacheFactory, ProjectCachePort } from '../application/ports.ts';
...
export const createProjectCache = (identity: string, normalizeState: (state: AppState) => AppState, browser: Pick<Window, 'indexedDB' | 'localStorage'> = window): ProjectCachePort => {
...
export class ProjectCacheWriter {
  ...
  private readonly cache: ProjectCachePort;
  private readonly onStatus: (status: CacheWriteStatus, projectId: string, error?: ProjectCacheError) => void;
  private readonly delayMs: number;

  constructor(cache: ProjectCachePort, onStatus: (status: CacheWriteStatus, projectId: string, error?: ProjectCacheError) => void, delayMs = 120) {
    this.cache = cache;
...
export const projectCacheFactory: ProjectCacheFactory = {
  create(identity, normalizeState, onStatus) {
    const cache = createProjectCache(identity, normalizeState);
    const writer = new ProjectCacheWriter(cache, onStatus);
    return {
      load: (projectId) => cache.load(projectId),
      schedule: (project) => writer.schedule(project),
      flush: () => writer.flush(),
    };
  },
};
